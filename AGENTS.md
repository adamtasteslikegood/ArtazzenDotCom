...
app/
  ...
  print_master.py        # Opt-in Real-ESRGAN upscaling to 300 DPI print masters
  ...
  security.py            # Basic auth dependency, security headers, private static guard
...
Layering (no cycles): `config → sidecars → ai_metadata → print_master → curation → watcher → seo → security/routes → factory → main`. Cross-module calls go through module attributes (e.g. `config.IMAGES_DIR`) so tests monkeypatch the defining module.
...
- **Print masters**: Opt-in (`upscale_enabled`, default off) Real-ESRGAN upscaling of an image into a 300 DPI PNG under `IMAGES_DIR/print_masters/`, tracked in the sidecar's optional `print_master` block. Backends: Replicate hosted (`REPLICATE_API_TOKEN`), `realesrgan-ncnn-vulkan` binary (`REALESRGAN_BIN`), or local torch. Masters are the sellable full-resolution asset: the static mounts refuse `print_masters/` (and the internal `.trash/` and `.curation/` directories), and only the authenticated `/admin/print-master/{image}/file` route serves them. See `PRINT_WORKFLOW.md`.
...
| `/admin/print-master/{image_name}`      | GET/POST | Print-master status / generate (`force=true` regenerates)         |
| `/admin/print-master/{image_name}/file` | GET      | Download the 300 DPI master (admin-only; never served statically) |
...
| `UPSCALE_ENABLED`             | `0`                    | Auto-generate a print master after upload/import (opt-in)              |
| `REPLICATE_API_TOKEN`         | _(none)_               | Enables the hosted Replicate upscale backend (recommended on Railway)  |
